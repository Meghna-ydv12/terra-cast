...
class ExpertSystemService:
    def generate_policy_brief(self, aqi, category, top_features, extended_context):
        ...
        status = "NORMAL"
        headline = f"⚡ [TerraCast Rule Engine] The simulated AQI has shifted to {aqi} ({category}), driven primarily by elevated {top_factor} levels."
        interventions = []
        secondary_alerts = []
        ...
        if aqi <= 100:
            ...
            status = "ELEVATED"
            headline = f"⚡ [TerraCast Rule Engine] With {top_factor} dominating the predictive model, we advise a localized Tier-1 intervention."
            ...
            status = "CRITICAL"
            headline = f"⚡ [TerraCast Rule Engine] Extreme {category} conditions detected. {top_factor} levels are severely impacting regional air quality."
